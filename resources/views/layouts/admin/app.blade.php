...
<body class="footer-offset">
                            ...
                            <div class="col-12">
                                <center>
                                    <h2 style="color: rgba(96,96,96,0.68)">
                                        ...
                                        {{ translate('You have new notification cash payment, Check Please.') }}
                                    </h2>
                                    <hr>
                                    <button onclick="check_notification()"
                                        class="btn btn-primary">{{ translate('Ok, let me check') }}</button>
                                </center>
                            </div>
                        </div>
                        ...
                        <div class="max-349 mx-auto mb-20">
                            ...
                            <div class="btn--container justify-content-center">
                                <button type="button" id="toggle-status-ok-button" class="btn btn-primary min-w-120"
                                    data-dismiss="modal" onclick="confirmStatusToggle()">{{ translate('Ok') }}</button>
    ...
    <script>
        ...
        function check_order() {
            location.href = '{{ route('admin.table.order.list', ['status' => 'all']) }}';
        }
    ...
    {{-- <script>
                ...
                $.get({
                    ...
                    success: function(response) {
                        ...
                        if (data.new_notification > 0) {
                            playAudio();
                            $('#notification-modal').appendTo("body").modal('show');
                        }
                    },
                });
            }, 10000);
        ...
        function check_notification() {
            location.href = '{{ route('admin.notification') }}';
        }
   ...
   <script>
        ...
        $.get({
            ...
            success: function(response) {
                ...
                if (data.new_notification > 0) {
                    ...
                    let notificationId = data.notifications.length > 0 ? data.notifications[0].id : null;

                    if (notificationId) {
                        $.post('{{ route('admin.mark-notification-as-read') }}', { notification_id: notificationId });
    ...
    function check_notification() {
        location.href = '{{ route('admin.verify-offline-payment', ['status' => 'all']) }}';
    }
