        ...
        <div>
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title c1">{{ translate('welcome') }}, {{ auth('admin')->user()->f_name }}
                        {{ auth('admin')->user()->l_name }}.</h1>
                    ...
                    <div class="col-auto">
                        <h4 class="d-flex align-items-center gap-10 mb-0">
                            <img width="20" class="avatar-img rounded-0"
                                src="{{ asset('assets/admin/img/icons/business_analytics.png') }}" alt="Business Analytics">
                    ...
                    <div class="col-auto">
                        <select class="custom-select min-w200" name="statistics_type"
                            ...
                            <option value="today"
                                ...
                                {{ \App\CentralLogics\translate('Today') . "'s" }}
                                {{ \App\CentralLogics\translate('Statistics') }}
                            </option>
                            <option value="this_month"
                                {{ session()->has('statistics_type') && session('statistics_type') == 'this_month' ? 'selected' : '' }}>
                                {{ \App\CentralLogics\translate('This Month') . "'s" }}
                                {{ \App\CentralLogics\translate('Statistics') }}
                            </option>
                        </select>
                    </div>
            ...
            <div class="grid-chart mb-3">
                                ...
                                @forelse ($table as $value)
                                    @if ($value->booking == 1)
                                        <div class="col-md-6 col-lg-3" style="padding: 10px;">
                                            ...
                                            <a href="#" class="dashboard--card" style="background-color: green">
                                                <h5 class="dashboard--card__subtitle" style="color: #fff">Table No</h5>
                                                
                                                <h2 class="dashboard--card__title" style="color: #fff">
                                                    {{ $value->number ?? 0 }}</h2>
                                                     <h5 class="dashboard--card__subtitle" style="color: #fff;text-align: end;">{{ucfirst($value->orderstatus->order_status ?? '')}}
                                                  
                                                    </h5>


                                                <img width="30"
                                                    src="{{ asset('assets/admin/img/icons/returned.png') }}"
                                                    class="dashboard--card__img" alt="">
                                            </a>
                                    ...
                                    @else
                                        <div class="col-md-6 col-lg-3" style="padding: 10px;">
                                            ...
                                            <a href="#" class="dashboard--card">
                                                <h5 class="dashboard--card__subtitle">Table No</h5>
                                                <h2 class="dashboard--card__title">{{ $value->number ?? 0 }}</h2>
                                                <img width="30"
                                                    src="{{ asset('assets/admin/img/icons/returned.png') }}"
                                                    class="dashboard--card__img" alt="">
                                            </a>
                ...
                <div class="card h100 recent-orders">
                    ...
                    <div class="card-header d-flex justify-content-between gap-10">
                        ...
                        <a href="{{ route('admin.table.order.list', ['all']) }}"
                            ...
                            @foreach ($data['recent_orders'] as $recent)
                                <li class="pt-0 d-flex flex-wrap gap-2 align-items-center justify-content-between">
                                    <div class="order-info ">
                                        <h5><a href="{{ route('admin.orders.details', ['id' => $recent->id]) }}"
                                                class="text-dark">{{ translate('Order') }}# {{ $recent->id }}</a>
                                        </h5>
                                        <p>{{ \Illuminate\Support\Carbon::parse($recent->created_at)->format('d-m-y, h:m A') }}
                                        </p>
                                    </div>
                                    ...
                                    @elseif(
                                        $recent['order_status'] == 'confirmed' ||
                                            $recent['order_status'] == 'processing' ||
                                            $recent['order_status'] == 'out_for_delivery')
...
@push('script_2')
    ...
    <script>
        var options = {
            series: [{{ $donut['ongoing'] }}, {{ $donut['delivered'] }}, {{ $donut['pending'] }},
                {{ $donut['canceled'] }}, {{ $donut['returned'] }}, {{ $donut['failed'] }}
            ],
            chart: {
                width: 256,
                type: 'donut',
            ...
            labels: ['{{ translate('ongoing') }}', '{{ translate('delivered') }}', '{{ translate('pending') }}',
                '{{ translate('canceled') }}', '{{ translate('returned') }}',
        ...
        var earningOptions = {
            ...
            series: [{
                ...
                data: [{{ $earning[1] }}, {{ $earning[2] }}, {{ $earning[3] }}, {{ $earning[4] }},
                    {{ $earning[5] }}, {{ $earning[6] }},
                    {{ $earning[7] }}, {{ $earning[8] }}, {{ $earning[9] }}, {{ $earning[10] }},
                    {{ $earning[11] }}, {{ $earning[12] }}
                ],
            }, ],
            markers: {
                size: 2,
