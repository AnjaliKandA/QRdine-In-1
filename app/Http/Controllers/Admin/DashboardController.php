...
//            ->whereBetween('created_at', [$from, $to])
        ...
        $ldate = date('Y-m-d');
        $donut['sales'] = $donut_data->where('order_date',$ldate)->sum('order_amount');
        
        $data['recent_orders'] = $this->order->latest()->take(5)->get();
        $table=Table::with('orderstatus')->get();

       

        return view('admin-views.dashboard', compact('data', 'earning', 'order_statistics_chart', 'donut','table'));
    }
...
//                ->whereBetween('created_at', [$from, $to])
 ...
 public function notification(Request $request)
    {
        // MiscHelper::user_notification('success', 'Notification content');

        // return response()->json(['message' => 'Notification triggered successfully']);
        $search = $request->input('search');
        $orderQuery = NotificationTemplate::query();
        ...
        $notifications = $orderQuery->orderBy('id', 'DESC')->get();
        return view('admin-views.notification_template.index', compact('notifications', 'search'));
    }

    public function getNotificationCount()
    {
        $totalNotificationCount = NotificationTemplate::count();

        return response()->json(['count' => $totalNotificationCount]);
    }
