...
class OrderController extends Controller
{
    ...
    public function status(Request $request): RedirectResponse
    {
        ...
        if ($request->order_status == 'completed' && $order->payment_status == 'paid') {
   DB::table('preference')
        ->where('order_no', $order->ordercode)  
       ->update(array('orderstatus' => 0));
       
             DB::table('tables')
         ->where('number',$order->table_id) 
        
         ->update(array('booking' => 0));

       DB::table('tables')->where('id',$order->table_id)->update(array('booking' => 0));
    ...
    public function verify_offline_payment($order_id, $status): JsonResponse
    {
        ...
        if ($offline_data->status == 1){
            ...
            if ($local != 'en'){
                $status_key = Helpers::order_status_message_key('confirmed');
                $translated_message = $this->business_setting->with('translations')->where(['key' => $status_key])->first();
                if (isset($translated_message->translations)){
                    foreach ($translated_message->translations as $translation){
                        if ($local == $translation->locale){
