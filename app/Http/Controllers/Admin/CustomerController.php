...
class CustomerController extends Controller
{
    ...
    public function customer_list(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $customers = $this->customer_address->where(function ($q) use ($key) {
                foreach ($key as $value) {
                    $q->orWhere('contact_person_name', 'like', "%{$value}%")
                        ->orWhere('contact_person_number', 'like', "%{$value}%")
                        ->orWhere('order_no', 'like', "%{$value}%");
                }
            });
            $query_param = ['search' => $request['search']];
        } else {
            $customers = $this->customer_address;
        }
        ...
        foreach ($customers as $customer) {
            $orderCounts[$customer->contact_person_number] = CustomerAddress::where('contact_person_number', $customer->contact_person_number)
                ->where('contact_person_name', $customer->contact_person_name)
                ->count();
        }

        return view('admin-views.customer.list', compact('customers', 'search', 'orderCounts'));
    }
    ...
    {
        try {
            $this->customer_address->findOrFail($request['id'])->delete();
