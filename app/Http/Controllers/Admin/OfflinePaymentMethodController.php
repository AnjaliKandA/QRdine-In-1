...
class OfflinePaymentMethodController extends Controller
{
    ...
    public function offline_payment_list(Request $request, $status): Factory|View|Application
    {
        ...
        $status = isset($status_mapping[$status]) ? $status_mapping[$status] : null;

        $orders = $this->order->with('customerAddress')
            ...
            ->orderBy('created_at', 'desc')->paginate(Helpers::getPagination());
    ...
    public function verifyOfflinePayment(Request $request, $orderId)
    {
        try {
